...
dev-install-mcp: build-nix
  # `dev-mcp` writes a project-local .mcp.json with a `chrest-dev`
  # server key pointing at result/bin/chrest (the nix store path).
  # That gives us a separate MCP entry alongside the production
  # `chrest`, so we can A/B against the dev binary without
  # overwriting the global ~/.claude.json. The .mcp.json must be
  # generated by the same binary it points at because dev-mcp
  # resolves its own executable path.
  result/bin/chrest dev-mcp
