...
async function makeUrlItems(items) {
...
async function makeUrlItem(item) {
  let result = item;
...
async function removeUrlItems(items) {
  if (items === undefined) {
    return [];
  }
...
Routes["/items"] = {
  async get(req) {
    return {
      ...
      body: [
        await allTabItems(),
        await allBookmarkItems(),
        await allHistoryItems(),
      ].flat(),
    };
  },
  async put(req) {
    return {
      body: {
        added: await makeUrlItems(req.body.added),
        deleted: await removeUrlItems(req.body.deleted),
      },
      status: 200,
    };
  },
